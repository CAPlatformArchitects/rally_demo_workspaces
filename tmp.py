...
def main(args):
	...
	print "username is now " + user_name
        ...
        try:
		rally = Rally(rally_server, user_name, "Kanban!!", workspace=workspace, project=project)
	...
	rally.setWorkspace("ZDeleteME")
	rally.setProject("Payment Team")
	data = {'FormattedID' : 'US1', 'Successors' : '173574263636'}

	preds = []
	story = rally.update('Story', data)
	print story.details()

	story1 = rally.get('Story', fetch='true', query="FormattedID = US145")
	t = story1.next()
	t.details()
	story2 = rally.get('Story', fetch='true', query="FormattedID = US144")
	s = story2.next()
	print s.details()
	...
	adds = rally.addCollectionItems(story, preds)
	print (adds)
